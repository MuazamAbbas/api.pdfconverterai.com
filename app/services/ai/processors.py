"""Concrete Processor for the `ai` module's Tier 2 job (Handbook Part C.4,
...
`app/worker.py`'s `ai_keyword_research` task function is the thin
ARQ-facing wrapper that calls `.run()` and owns the Job's
...
This is the first Processor filed under `app/services/ai/` - Keyword
Research (ADR-018) is the first Tier 2 AI tool; Grammar Checker
